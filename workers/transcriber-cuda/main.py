#!/usr/bin/env python3
"""CUDA song transcriber: faster-whisper lyrics detection for the kelvin model."""
import asyncio
import json
import logging
import os
...
logger = logging.getLogger("worker.transcriber-cuda")

QUEUE = "queue:transcriber-cuda"
...
async def process_lyrics(r, db, task_id: str, song_id: str, language: str):
    ...
    await _publish(r, task_id, "running", "transcribing", 0.0, message="Starting faster-whisper transcription...")
...
async def main():
    ...
    logger.info(f"CUDA transcriber ready — listening on {QUEUE}")

    while True:
        try:
            _, raw = await r.brpop(QUEUE, timeout=0)
            job = json.loads(raw)
            task_id = job["task_id"]
            song_id = job["song_id"]
